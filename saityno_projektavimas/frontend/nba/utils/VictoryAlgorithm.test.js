...
test('Test calculatePoints function', () => {
    ...
    let teamPts = parseFloat(
        obj.rpg * rebounds_points + obj.bpg * blk_points + obj.apg * assists_points
    ).toFixed(2);
...
test('Add player stats function', () => {
    ...
    expect(AddPlayerStats(newObj, objToAdd)).toEqual({
        fg: 15.7,
        threePt: 15.7,
        ft: 15.7,
        ppg: 15.7,
...
test('Calculate points function', () => {
    ...
    expect(RecalculateStats(objToAdd, length)).toEqual({
        fg: 10.1,
        threePt: 10.1,
        ft: 10.1,
        ppg: 10.1,
